...
<table class="topic_table" border="0" cellspacing="0">
    ...
    <tbody>
        ...
        <tr class="postinfo">
            ...
            <td width="50%">{{ helper:date format="D M j Y - g:i a" timestamp="<?php echo $post['created'] ?>"}}</td>
                <?php if($post['parent_id'] == 0): ?>
            <td width="30%" class="postreport">
		[ <?php echo anchor('forums/posts/report/'.$post['id'], 'Report');?> ]
                        <?php if($this->ion_auth->is_admin() && !$topic->is_sticky): ?>
		[ <?php echo anchor('forums/topics/stick/'.$post['id'], 'Make Sticky');?> ]
                        <?php elseif($this->ion_auth->is_admin() && $topic->is_sticky ): ?>
        ...
        <tr>
            <td valign="top" class="authorinfo">
                <a href="<?php echo site_url('user/'.$post['created_by']['user_id']); ?>">
                        ...
                        {{ if avatar }}{{ avatar:img }} {{ else }}  <?php echo gravatar($post['created_by']['email']); ?>{{ endif }}
                        {{ /user:profile }}

                </a>

                <p>
			Joined: <br><?php /* echo date("M j Y", $post['created_by']['created_on']); */ ?>
                </p>
                <p>
			Posts:
		...
		[ <?php echo anchor('messages/create/'.$post['created_by']['user_id'], 'Message');?> ]
            ...
            <td colspan="2" align="right">[ <?php echo anchor('forums/posts/quote_reply/'.$post['id'], 'Quote');?> ]
                    <?php
                        ...
                        //if((isset($user->id) && $post['created_by']['user_id'] == $user->id) || $this->ion_auth->is_admin()):
                    ?>
	 [ <?php echo anchor('forums/posts/edit_reply/'.$post['id'], 'Edit');?> ]
                    <?php endif; ?>
                    <?php
                        if($this->ion_auth->is_admin()):
                        //if(((isset($user->id) && $post['created_by']['user_id'] == $user->id) && $post->parent_id != 0) || $this->ion_auth->is_admin()):
                    ?>
